...
mod platform {
    ...
    use super::super::model::{Error, ProcessContext, Result, PROCESS_CTX_VERSION, SIGNATURE};
    ...
    use libc::timespec;
    use libc::{
        c_void, close, ftruncate, madvise, mmap, munmap, prctl, MADV_DONTFORK,
        MAP_ANONYMOUS, MAP_FAILED, MAP_PRIVATE, MAP_SHARED, PROT_READ, PROT_WRITE,
    };
    use tracing::info;
    ...
    /// Get the mapping size (just the header — mmap rounds up to a page).
        ...
        pub fn drop_context(&mut self) -> Result<()> {
            ...
            if let Some(handle) = guard.take() {
                ...
                if current_pid == handle.publisher_pid
                    && unsafe { munmap(handle.mapping, handle.mapping_size) } == -1
                {
...
#[cfg(target_os = "linux")]
pub use platform::ProcessContextWriter;

// Non-Linux stub implementation
#[cfg(not(target_os = "linux"))]
mod stub {
    use super::super::model::{Error, ProcessContext, Result};
    use tracing::info;
...
#[cfg(not(target_os = "linux"))]
pub use stub::ProcessContextWriter;
